...
function VideoSideRailCard({
  ...
  return (
    <div
      className={`group overflow-hidden rounded-xl border border-white/80 shadow-md shadow-brand-navy/10 ring-1 ring-white/30 ${className}`.trim()}
    >
      <HomeDecorativeImage
        src={src}
...
export function VideoFirstSection() {
  ...
  return (
    <section
      id="home-video"
      className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm md:rounded-3xl"
    >
      <div className="pointer-events-none absolute -left-8 top-8 h-24 w-24 rounded-full bg-brand-gold-light blur-2xl opacity-80 home-float-a" />
      <div className="pointer-events-none absolute right-10 bottom-8 h-28 w-28 rounded-full bg-slate-100 blur-2xl home-float-b" />
      <Row gutter={0}>
        <Col
          xs={24}
          md={9}
          lg={8}
          className="relative flex min-h-[220px] flex-col justify-center overflow-hidden border-b border-white/10 bg-linear-to-br from-brand-navy via-brand-gold-dark to-brand-violet-bright/30 p-8 text-white md:min-h-[300px] md:border-b-0 md:border-r md:py-12"
        >
          <div className="pointer-events-none absolute -right-6 bottom-0 h-40 w-40 rounded-full bg-brand-gold/25 blur-3xl home-glow-pulse" />
          <div className="pointer-events-none absolute inset-0 hidden md:block" aria-hidden>
            <div className="absolute -right-1 top-[14%] w-[min(46%,150px)] rotate-[7deg] overflow-hidden rounded-xl border-[3px] border-white/25 shadow-xl shadow-black/25 ring-1 ring-white/20 home-float-a">
              <HomeDecorativeImage
                src={left0.src}
            ...
            <div className="absolute bottom-[10%] left-0 w-[min(42%,130px)] -rotate-[5deg] overflow-hidden rounded-xl border-[3px] border-white/20 shadow-lg shadow-black/20 ring-1 ring-white/15 home-float-b">
              <HomeDecorativeImage
                src={left1.src}
          ...
          <div className="relative z-[1]">
            <Typography.Text className="relative mb-2 block text-[11px] font-bold uppercase tracking-[0.2em] text-white/90">
              Video delivery
            </Typography.Text>
            <span className="relative mb-4 block h-1 w-10 rounded-full bg-linear-to-r from-white/90 via-brand-gold to-white/70" aria-hidden />
        ...
        <Col xs={24} md={15} lg={16} className="p-6 md:p-8 lg:p-10">
          ...
          <div className="mt-4 flex flex-col gap-4 lg:mt-6 lg:flex-row lg:items-stretch">
            ...
            <div
              ref={mediaRef}
              className={`group relative min-w-0 flex-1 overflow-hidden rounded-2xl border-2 border-slate-200 bg-white shadow-lg shadow-slate-200/60 ring-1 ring-slate-100 home-float-c home-media-enter ${
            ...
            >
              ...
              >
                <div className="home-video-play-ring relative flex h-[4.75rem] w-[4.75rem] items-center justify-center rounded-full shadow-[0_12px_40px_rgba(10,37,64,0.4)] ring-[3px] ring-white/90 transition-transform duration-300 ease-out group-hover:scale-110 motion-reduce:transition-none">
                  <span className="absolute inset-0 rounded-full bg-linear-to-br from-white via-white to-slate-100" />
                  <span className="absolute inset-[-6px] rounded-full border-2 border-white/40 opacity-90" />
